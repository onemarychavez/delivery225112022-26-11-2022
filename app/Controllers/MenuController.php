...
class MenuController extends BaseController
{
        ...
        try {
            ...
            foreach($menues as $menu){
                array_push($data,[
                    ...
                    'nombre'=>$menu['nombre'],
                    'keycategoria'=>intval($menu['idcategoria']),
                    'menu'=>$this->getMenuByEMpresa($menu['idmenu'])
                ]);
            }
            return $this->response->setStatusCode(200,'OK')
            ->setJSON($data);
    ...
    private function getMenuByEMpresa($menu){
        $menu = $this->mdetalle->select("menu_detalle.*,menu.idmenu as m")
        ->join("menu","menu_detalle.idmenu=menu.idmenu","inner")
        ->where([
            "menu.idmenu"=>$menu,
            "menu_detalle.activo"=>true,
        ])->findAll();
